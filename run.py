...
def scrape_following(bot, user, user_input):
    ...
    time.sleep(2)
    print(f"[Info] - Scraping following for {user}...")
    ...
    print(f"[Info] - Saving followers for {user}...")
    with open(f'{user}_following.txt', 'a') as file:
        file.write('\n'.join(users) + "\n")
...
def scrape():
    ...
    user_input2 = int(input('[Required] - How many following do you want to scrape (100-2000 recommended): '))
    ...
    for user in usernames:
        ...
        scrape_following(bot,user, user_input2)

    for user in usernames:
        input1 = int(
            input("Do you want to know who doesn't follow you? Enter 1 for yes, Enter any other character to stop."))

        if input1 == 1:
            def find_unfollowers(followers_file, following_file, output_file):
                try:
                    with open(followers_file, 'r') as f1:
                        followers = set(f1.read().split())

                    with open(following_file, 'r') as f2:
                        following = set(f2.read().split())

                    not_followed_back = following.difference(followers)

                    with open(output_file, 'w') as output_f:
                        output_f.write('\n'.join(not_followed_back))

                    print(f"Users who don't follow back saved in {output_file}")

                except FileNotFoundError:
                    print("One or more files not found. Please check the file paths.")

            # Specify the file paths
            followers_file_path = f'{username}_followers.txt'
            following_file_path = f'{username}_following.txt'

            open(f"{username}_output", "w").close()
            output_file_path = f'{username}_output.txt'





            # Call the function to find users who don't follow back
            find_unfollowers(followers_file_path, following_file_path, output_file_path)
